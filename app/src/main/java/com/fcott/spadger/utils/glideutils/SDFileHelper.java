...
public class SDFileHelper {
    ...
    //Glide保存图片
    public void savePicture(final String firstF,final String secondF,final String fileName, String url){
        Glide.with(context).load(url).asBitmap().toBytes().into(new SimpleTarget<byte[]>() {
            @Override
            public void onResourceReady(byte[] bytes, GlideAnimation<? super byte[]> glideAnimation) {
                try {
                    savaFileToSD(firstF,secondF,fileName,bytes);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    public void savaFileToSD(String firstF,String secondF,String filename, byte[] bytes) throws Exception {
        //如果手机已插入sd卡,且app具有读写sd卡的权限
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            String filePath = Environment.getExternalStorageDirectory().getCanonicalPath()+"/Handout"+"/"+firstF+"/"+secondF;
            File dir1 = new File(filePath);
            if (!dir1.exists()){
                dir1.mkdirs();
            }
