...
public abstract class BaseAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private boolean onConnectedHasNeedFresh = false;//网络连接时是否刷新
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    if (!isAutoLoadMore && findLastVisibleItemPosition(layoutManager) + 1 == getItemCount()) {
                        onConnectedHasNeedFresh = true;
                        scrollLoadMore();
                    }else {
                        onConnectedHasNeedFresh = false;
                    }
                }
            }
    ...
    private void scrollLoadMore() {
        if (mFooterLayout.getChildAt(0) == mLoadingView) {
            mLoadMoreListener.onLoadMore(LoadMode.LOADMORE);
        }
    }
    ...
    public void setLoadFailedView(View loadFailedView) {
        ...
        mLoadFailedView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                addFooterView(mLoadingView);
                mLoadMoreListener.onLoadMore(LoadMode.RELOAD);
            }
        });
    }
    ...
    public boolean onConnectedHasNeedFresh(){
        if (mFooterLayout.getChildAt(0) == mLoadingView) {
            return onConnectedHasNeedFresh;
        }
        return false;
    }
